...
/**
 * Medications Page (Server Component)
 *
 * SSR Benefits:
...
export const metadata = {
  title: 'Medications | Xoai Healthcare',
  description: 'Patient medications and prescriptions management',
};
...
export default async function MedicationsPage() {
  ...
  const session = await requireSession('/dashboard/medications');

  // Log PHI access for HIPAA audit trail
  await logPHIAccess('VIEW', 'Medication', 'list', {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-slate-900">Medications</h1>
      ...
      {/* Medication List (Client Component with server-fetched patient list) */}
      <Suspense fallback={<MedicationListSkeleton />}>
        <MedicationList patients={patients} />
      </Suspense>
    </div>
  );
}
